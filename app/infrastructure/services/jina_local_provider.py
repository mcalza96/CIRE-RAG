...
class JinaLocalProvider(IEmbeddingProvider):
    ...
    def _load_model(self):
        with self._lock:
            ...
            logger.info(f"⏳ [JinaLocalProvider] Loading model {self._model_name} on {self.device}...")
            try:
    ...
    async def embed(self, texts: List[str], task: str = "retrieval.passage") -> List[List[float]]:
        ...
        logger.info(f"🔢 [JinaLocalProvider] Generating embeddings for {len(texts)} texts (task: {task})...")
    ...
    async def chunk_and_encode(self, text: str) -> List[Dict[str, Any]]:
        ...
        while current_char < text_len:
            ...
            window_spans = [s for s in spans if s[0] >= current_char and s[0] < window_end and s not in seen_spans]
    ...
    def _process_window(self, full_text: str, window_spans: List[Tuple[int, int]], window_start: int, window_end: int) -> List[Dict[str, Any]]:
        ...
        for start_char_abs, end_char_abs in window_spans:
            ...
            token_indices = np.where((offset_mapping[:, 1] > start_char_rel) & (offset_mapping[:, 0] < end_char_rel))[0]
            if len(token_indices) == 0: 
                continue
                
            start_token = token_indices[0]
            end_token = token_indices[-1] + 1
            pooled_vector = token_embeddings[start_token:end_token].mean(dim=0).cpu().numpy().tolist()

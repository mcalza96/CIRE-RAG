...
class ISourceRepository(ABC):
    ...
    async def update_status_and_metadata(
        self, doc_id: str, status: str, metadata: Dict[str, Any]
    ) -> None:
        pass
...
class IPageLocator(Protocol):
    def get_page_number(self, char_idx: int, page_map: List[Dict[str, Any]]) -> int: ...


class IChunkEmbeddingService(Protocol):
...
class ITextEmbeddingService(Protocol):
    async def embed_texts(
        self,
        texts: List[str],
        task: str = "retrieval.passage",

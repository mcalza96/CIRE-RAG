...
async def get_ingestion_batch_progress(
    ...
    try:
        ...
        if detail == "BATCH_NOT_FOUND":
            raise ApiError(status_code=404, code="BATCH_NOT_FOUND", message="Batch not found", details=detail)
        raise ApiError(status_code=400, code="INVALID_BATCH_PROGRESS_REQUEST", message="Invalid batch progress request", details=detail)
    except Exception as e:
        logger.error("get_batch_progress_failed", batch_id=batch_id, error=str(e))
        raise ApiError(status_code=500, code="GET_BATCH_PROGRESS_FAILED", message="Get batch progress failed")
...
async def get_ingestion_batch_events(
    ...
    try:
        ...
        if detail == "BATCH_NOT_FOUND":
            raise ApiError(status_code=404, code="BATCH_NOT_FOUND", message="Batch not found", details=detail)
        raise ApiError(status_code=400, code="INVALID_BATCH_EVENTS_REQUEST", message="Invalid batch events request", details=detail)
    except Exception as e:
        logger.error("get_batch_events_failed", batch_id=batch_id, error=str(e))
        raise ApiError(status_code=500, code="GET_BATCH_EVENTS_FAILED", message="Get batch events failed")
...
async def list_active_ingestion_batches(
    ...
    try:
        ...
        if "TENANT_MISMATCH" in detail:
            raise ApiError(status_code=400, code="TENANT_MISMATCH", message="Tenant mismatch", details=detail)
        raise ApiError(status_code=400, code="INVALID_ACTIVE_BATCHES_REQUEST", message="Invalid active batches request", details=detail)
    except Exception as e:
        logger.error("list_active_batches_failed", tenant_id=tenant_id, error=str(e))
        raise ApiError(status_code=500, code="LIST_ACTIVE_BATCHES_FAILED", message="List active batches failed")
...
async def stream_ingestion_batch(
    ...
    async def _event_stream():
        ...
        while True:
            ...
            delta = await use_case.get_batch_events(batch_id=batch_id, cursor=current_cursor, limit=100)

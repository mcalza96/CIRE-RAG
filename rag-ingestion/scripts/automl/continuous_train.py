...
PROD_MODEL_PATH = os.path.join(os.path.dirname(__file__), "../../app/core/prompts/optimized_socratic.json")
...
async def main():
    ...
    # 0. Setup LM
    # Using Groq (Llama 3.3 70b) for high-quality retraining
    lm = dspy.Groq(model="llama-3.3-70b-versatile", max_tokens=2000)
    ...
    evaluator = Evaluate(devset=devset, metric=socratic_assessment_metric, num_threads=4, display_progress=False)
    
    score_champion = evaluator(champion)
    score_challenger = evaluator(challenger)
    
    logger.info(f"ARENA RESULTS: Champion={score_champion:.2f} vs Challenger={score_challenger:.2f}")

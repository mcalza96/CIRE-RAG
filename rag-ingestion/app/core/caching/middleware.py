...
def cached_extraction(func: AsyncMethod[VisualParseResult]) -> AsyncMethod[VisualParseResult]:
    ...
    async def wrapper(self: Any, *args: Any, **kwargs: Any) -> VisualParseResult:
        ...
        cache_row = await _fetch_cache_row(image_hash=image_hash, provider=provider, model_version=model_version)
        ...
        if cache_row is not None:
            ...
            return VisualParseResult.model_validate(result_data)
...
async def _fetch_cache_row(image_hash: str, provider: str, model_version: str) -> dict[str, Any] | None:

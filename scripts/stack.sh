...
start_all() {
  ...
  start_process "Community Worker" "venv/bin/python -m app.infrastructure.background_jobs.community_worker" "$RAG_DIR" "$COMMUNITY_WORKER_PID_FILE" "$COMMUNITY_WORKER_LOG"
}
...
stop_all() {
  ...
  pkill -f "app.infrastructure.background_jobs.community_worker" 2>/dev/null || true
}

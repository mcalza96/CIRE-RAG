...
-- Align GraphRAG vectors with runtime Jina v3 dimensions (1024).

DROP INDEX IF EXISTS idx_knowledge_entities_embedding_hnsw;
DROP INDEX IF EXISTS idx_knowledge_relations_embedding_hnsw;
DROP INDEX IF EXISTS idx_knowledge_communities_embedding_hnsw;
...
ALTER TABLE public.knowledge_entities
    ...
    USING (
        CASE
            ...
            WHEN vector_dims(embedding) > 1024 THEN subvector(embedding, 1, 1024)::vector(1024)
            ELSE NULL
        END
    );
...
DROP FUNCTION IF EXISTS public.match_knowledge_entities(vector(1536), uuid, int, float);
DROP FUNCTION IF EXISTS public.match_knowledge_communities(vector(1536), uuid, int, int, float);
...
DROP FUNCTION IF EXISTS public.hierarchical_search_batch(text[], vector(1536)[], float, int, uuid, ltree);
